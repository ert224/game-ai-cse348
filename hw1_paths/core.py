...
class ManualObstacle(Obstacle):
	...
	def __init__(self, points, color = (0, 0, 0), linewidth = 4, sprite = None):
		Obstacle.__init__(self)
		print("pts", points)
		minpt = ( min(list(map(lambda p: p[0], points))), 
			min(list(map(lambda p: p[1], points))) )
		maxpt = ( max(list(map(lambda p: p[0], points))), 
			max(list(map(lambda p: p[1], points))) )
		# create surface
		...
		if sprite is not None:
			...
			pos = (0, 0)
			for x in range(int((self.rect.width*2)/dec.rect.width)):
				for y in range(int((self.rect.height*2)/dec.rect.height)):
...
#			self.sprites.add(n)
	...
	def computeFreeLocations(self, agent):
		if type(agent) not in self.destinations:
			...
			for x in range(1, int(self.dimensions[0]/grid)):
				for y in range(1, int(self.dimensions[1]/grid)):
					point = (x*grid, y*grid)
...
class GatedWorld(GameWorld):
	...
	def getGates(self):
		return list(map(getGateLine, self.gates))
	...
	def run(self):
		...
		game_world = self
		draw_iterator = threaded_draw_gates()
		update_iterator = threaded_update()
